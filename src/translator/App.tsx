...
const sendTranslationRequest = async (text: string, targetLanguage: string): Promise<TranslationResponse> => {
  ...
  if (!runtime?.sendMessage) {
    throw new Error('Chrome runtime 不可用，在扩展环境中打开此页面。');
  }
            ...
            <div className='flex flex-wrap items-end justify-between gap-4'>
              <div>
                ...
                <p className='text-sm text-[#9aa0a6]'>
                  沉浸式双栏体验 · Markdown & KaTeX 支持 · Volcengine Doubao
                </p>
              </div>
              <div className='flex items-center gap-6 text-sm text-[#9aa0a6]'>
                <span>
                  源语言：<strong className='text-white'>{getLabel(sourceLang)}</strong>
                </span>
                <span>
                  目标语言：<strong className='text-white'>{getLabel(targetLang)}</strong>
                </span>
              </div>
            </div>
          </div>
              ...
              <textarea
                ref={(textarea) => {
                  if (textarea) {
                    ...
                    // Set new height to match content, with a minimum of 420px
